...
/* Contour labels get buffered here.  */
...
static gaint splflg;                      /* Flag for yes/no spine fit  */
static gaint labtp;                       /* Label type                 */
...
void gxclev (gadouble *r, gaint is, gaint js, gaint ib, gaint ie, gaint jb,
   gaint je, gadouble v, char *u, char *clab, gaint spflag, gaint labtyp) {
...
strcpy (clabel,clab);                  /* Label for this contour     */
splflg = spflag;                       /* Spline fit flag            */
labtp = labtyp;                        /* Label type                 */
...
if (lwksiz<iw*jw*2) {                  /* If storage inadaquate then */
  ...
  lwk = (short *)malloc(lwksiz*sizeof(short));  /* Allocate flag work area */
  if (lwk==NULL) {
    printf ("Error in GXCLEV: Unable to allocate storage \n");
    return;
  }
  fwksiz = iw*jw;                      /* Size of coord pair buffer */
  fwk = (gadouble *)malloc(fwksiz*sizeof(gadouble));    /* Allocate fwk   */
  if (lwk==NULL) {
    printf ("Error in GXCLEV: Unable to allocate storage \n");
    free (lwk);
    lwk = NULL;
...
gadouble del,kurv,cmax,dacum,dcmin;
gadouble mslope,tslope,xlb=0.0,ylb=0.0;
gaint icls,nump,labflg;
gadouble *xy;

if (labtp==2) dacum=2.0;
...
while (x3 < xyend+2) {                 /* Loop to end of the line    */
  ...
  if (labtp && ((*y2-*y1)*(*y3-*y2)<0.0) && gxlabn<LABMAX && dacum>dcmin){
    if (clabel[0]) {
      gxlabx[gxlabn]=(*x2);
      gxlaby[gxlabn]=(*y2);
      gxlabs[gxlabn]=0.0;
  ...
  if (labtp==2 && !labflg && gxlabn<LABMAX && x2!=xyend) {
    if (*x1 < *x2 && *x2 < *x3) {
...
if (labtp==2 && !labflg && gxlabn<LABMAX && fabs(mslope)<2.0) {
  if (clabel[0]) {
    gxlabx[gxlabn]=xlb;
    gxlaby[gxlabn]=ylb;
...
/* Output all the labels in the label buffer, and blank beneath them.   */
