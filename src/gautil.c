...
/*ams 971228
      On MS-DOS with XLIBEMU we handle user input differently.
      See pcx11e.c for a nxtcmd() replacement. However, the utilities
      don't need this new user interface - macro STNDALN takes care
      of that.
ams*/

#if !defined(XLIBEMU) || defined(STNDALN)
...
/* dimprs parses a dimension 'expression', ie, where the user
   ...
   where:  dim = x,y,z,t,e,lat,lon,lev,time,ens
           op  = +, -, or =
           val = dimension value
...
char name[5],ename[16];
  ...
  if (i>4) {
    ...
    sprintf (pout,"  Expecting X/Y/Z/T/E/LAT/LON/LEV/TIME/ENS, found %s\n",name);
    gaprnt (0,pout);
    return (NULL);
  }
  ...
  if (cmpwrd("time",name)) {
    if (op==0) {
      ...
      if ((pos=rdtprs(ch,&dtim)) == NULL) return (NULL);
    }
  } 
  ...
  if (*dim < 5) {
    ...
    *wflag = 1;
	...
	i++; ens++;
      ...
      if (enum1<0) {
	gaprnt (0,"Syntax Error:  Invalid dimension expression\n");
	sprintf (pout,"  Ensemble name \"%s\" not found\n",ename);
	gaprnt (0,pout);
	return (NULL);
      }
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gamo32");
  if (vals==NULL) {
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gagaus");
  if (vals==NULL) {
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gags30");
  if (vals==NULL) {
    gaprnt (0,"Memory Allocation Error: Gaussian Grid Scaling\n");
    return (NULL);
  }
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gags20");
  if (vals==NULL) {
    gaprnt (0,"Memory Allocation Error: Gaussian Grid Scaling\n");
    return (NULL);
  }
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gags15");
  if (vals==NULL) {
    gaprnt (0,"Memory Allocation Error: Gaussian Grid Scaling\n");
    return (NULL);
  }
...
gadouble *vals;
  ...
  size = (num+3) * sizeof(gadouble);
  vals = (gadouble *)galloc(size,"gagst62");
  if (vals==NULL) {
    gaprnt (0,"Memory Allocation Error: Gaussian Grid Scaling\n");
    return (NULL);
  }
...
gaint gat2ch (struct dt *dtim, gaint tinc, char *ch) {
gaint mn1,mn2,hr1,hr2,dy1,dy2,len,mnth;
  ...
  if (tinc==1) {
    ...
    sprintf(ch,"%i%iZ%i%i%s%04i",hr1,hr2,dy1,dy2,monc[mnth],dtim->yr);
  }
  else if (tinc==5) {
    sprintf(ch,"%i%i:%i%iZ%i%i%s%04i",hr1,hr2,mn1,mn2,dy1,dy2,monc[mnth],dtim->yr);
  }
  else sprintf(ch,"???");
  len=0;
  while (ch[len]) len++;
...
gadouble *vvv;
 ...
 vvv = (gadouble *)galloc(sizeof(gadouble)*num,"cpscal");
 if (vvv==NULL) {
   sprintf(pout,"cpscal memory allocation error; dim=%d lin=%d num=%d\n",dim,lin,num);
   gaprnt(0,pout);
   return (NULL);
 }
 for (i=0; i<num; i++) {
