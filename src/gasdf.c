...
static char pout[1000];    /* Build error msgs here */
gaint utISinit = 0 ;
...
gaint gaxdfopen (char *args, struct gacmn *pcm) {
  ...
  rc = gadsdf(pfi, parms) ;
  if (rc==Failure) {
    sprintf (pout, "SDF Descriptor file %s was not successfully opened & parsed.\n", pfi->dnam) ;
    gaprnt(0, pout) ;
    frepfi (pfi, 0) ;
    freeparms(&parms);
    if (pcm->fnum <= 1) { 
...
gaint gadsdf(struct gafile *pfi, GASDFPARMS parms) {
  ...
  gadouble time1,time2,lat1,lat2,lev1,lev2,incrfactor;
  gafloat dsec;
  size_t len_time_units,trunc_point ;
  gaint size,len,noname,notinit,nolength;
  ...
  if (parms.needunpack) {
    ...
    if (attr) {
      len = strlen(attr->name);
      if ((pfi->scattr = (char *)galloc(len+1,"scattr1")) == NULL) goto err1;
    ...
    if (attr) {
      len = strlen(attr->name);
      if ((pfi->ofattr = (char *)galloc(len+1,"ofattr1")) == NULL) goto err1;
  ...
  if (parms.xsetup) {
	...
	return Failure ;
      ...
      if (!Xcoord) {
	sprintf (pout, "gadsdf: Can't find variable %s for X coordinate.\n",parms.xdimname);
	gaprnt(0,pout);
	return Failure ;
      }
    } 
  ...
  if (parms.isxdf && (!parms.xsrch)) {
    ...
    if (xdimid == -1) {
      sprintf (pout, "gadsdf: Lon dimension %s is not an SDF dimension.\n",parms.xdimname);
      gaprnt(0,pout);
      return Failure;
    }
  } else {
  ...
  if (parms.ysetup) {
	...
	return Failure ;
      ...
      if (!Ycoord) {
	sprintf (pout, "gadsdf: Can't find variable %s for Y coordinate.\n",parms.ydimname);
	gaprnt(0,pout);
	return Failure ;
      }
    } 
  ...
  if (parms.isxdf && (!parms.ysrch)) {
    ...
    if (ydimid == -1) {
      sprintf (pout,"gadsdf: Lat dimension %s is not an SDF dimension.\n",parms.ydimname);
      gaprnt(0,pout);
      return Failure;
    }
  } 
  ...
  if (parms.zsetup) {
    ...
    if (parms.zsrch) {
      ...
      if (!Zcoord) {
	sprintf (pout,"gadsdf: Can't find variable %s for Z coordinate.\n",parms.zdimname);
	gaprnt(0,pout);
	return Failure ;
      }
    }
    if (!Zcoord) {
      ...
      pfi->dnum[ZINDEX] = 1 ;
      if ((zvals = (gadouble *)galloc(sizeof(gadouble)*6,"zvals")) == NULL) {
  ...
  if (parms.isxdf && (!parms.zsrch)) {
    ...
    if (zdimid == -1) {
      sprintf (pout, "gadsdf: Lev dimension %s is not an SDF dimension.\n",parms.zdimname);
      gaprnt(0,pout);
      return Failure;
    }
  } else {
  ...
  if (parms.tsetup) {
    if (parms.tsrch) {
      ...
      if (!Tcoord) {
	sprintf (pout, "gadsdf: Can't find variable %s for T coordinate.\n",parms.tdimname);
	gaprnt(0,pout);
	return Failure ;
      }
    }
    if (!Tcoord) {
      ...
      pfi->dnum[TINDEX] = 1 ;
      if ((tvals = (gadouble *) galloc(sizeof(gadouble)*8,"tvals1")) == NULL) {
	...
	time2 = time1;
      ...
      if ((tvals = (gadouble *) galloc(sizeof(gadouble)*8,"tvals2")) == NULL) {
	...
	if (!strstr((char *)timeunits_attr->value, " since ")) {    
	  ...
	  time_units = (char *) galloc(len_time_units,"time_un1") ;
	  if (time_units==NULL) {
	  ...
	  time_units = (char *) galloc(len_time_units,"time_un2") ;
	  if (time_units==NULL) {
	    gaprnt(0, "gadsdf: Memory allocation error for time_units\n");
	    goto err2;
	  }
	...
	if (pfi->dnum[TINDEX] > 1) {
	  ...
	  if (!temp_str) {
	    ...
	    trunc_point = strlen(time_units)-strlen(temp_str)+1;
          ...
          trunc_units = (char *) galloc(trunc_point+1,"trunc_units");
	  if (trunc_units==NULL) {
  ...
  if (parms.isxdf && (!parms.tsrch)) {
    if (parms.tdimname != NULL) {
      ...
      if (tdimid == -1) {
	sprintf (pout, "gadsdf: Time dimension %s is not an SDF dimension.\n",parms.tdimname);
	gaprnt(0,pout);
	return Failure;
      }
    }
  ...
  if (parms.esetup) {
    ...
    /* find an E axis */
      ...
      if (!Ecoord) {
	sprintf (pout,"gadsdf: Can't find variable %s for Ensemble coordinate.\n",parms.edimname);
	gaprnt(0,pout);
	return Failure ;
      }
    } 
	...
	if ((evals = (gadouble *)galloc(sizeof(gadouble)*6,"evals")) == NULL) {
	...
	*(evals+5) = -999.9;
	...
	if ((ens = (struct gaens *)galloc(sizeof(struct gaens),"ens1")) == NULL) {
	...
	/* set up linear scaling */
	if ((evals = (gadouble *)galloc(sizeof(gadouble)*6,"evals1")) == NULL) {
	...
	*(evals+5) = -999.9;
	...
	size = pfi->dnum[EINDEX] * sizeof(struct gaens); 
	if ((ens = (struct gaens *)galloc(size,"ens2")) == NULL) {
	...
	while (i<=pfi->dnum[EINDEX]) {
	  if (noname) {
	    sprintf(ens->name,"%d",i);   /* default to ensemble index number for a name */
	  }
	  else {
  ...
  if (parms.isxdf && (!parms.esrch)) {
    ...
    if (edimid == -1) {
      sprintf (pout,"gadsdf: Ensemble dimension %s is not an SDF dimension.\n",parms.edimname);
      gaprnt(0,pout);
      return Failure;
    }
  } else {
  ...
  if (numdvars == 0) {
    ...
    if ((newpvar = (struct gavar *) galloc(numdvars*sizeof(struct gavar),"newpvar")) == NULL) {
  ...
  size = pfi->dnum[XINDEX] * sizeof(gadouble);
  if ((pfi->rbuf = (gadouble *)galloc(size,"rbuf")) == NULL) goto err1;
  if ((pfi->ubuf = (char *)galloc(size,"ubuf")) == NULL) goto err1;
...
gaint isdvar(struct gafile *pfi, struct gavar *var, 
  ...
  /* find out if it's a coordinate variable */
  iscoordvar = 0 ;
  if (var->nvardims == 1) {  /* varible is 1-D */
    for (i=0; i<pfi->nsdfdims; i++) {
      if (var->vardimids[0] == pfi->sdfdimids[i]) { /* variable dimid matches a file dimid */
	if (!strcmp(pfi->sdfdimnam[i], var->longnm)) { /* variable name matches dimension name */
	  iscoordvar = 1 ;
	}
      }
    }
  }
  if (iscoordvar) return Failure; 

  /* find out if it's coordinate dimensions match any of the 5 dimids */
  ...
  if (hasX || hasY || hasZ || hasT || hasE) {
    /* set the number of vertical levels for each variable */
    if (hasZ) {
...
gaint sdfdeflev(struct gafile *pfi, struct gavar *coord, gaint dim, gaint revflag) {
  ...
  int32 rank,natts,dim_sizes[MAX_VAR_DIMS],dtype,sds_id;
  int32 *idata=NULL;
  uint32 *uidata=NULL;
#endif
  ...
  len = pfi->dnum[dim] ; 
  if ((axisvals = (gadouble *)galloc(sizeof(gadouble)*len,"axisvals")) == NULL) {
  ...
  if (pfi->ncflg==2) {
    ...
    case (DFNT_INT32):    /* definition value 24 */
      if ((idata = (int32 *)galloc(len * sizeof (int32),"idata"))==NULL) {
    ...
    case (DFNT_UINT32):   /* definition value 25 */
      if ((uidata = (uint32 *)galloc(len * sizeof (uint32),"uidata"))==NULL) {
    ...
    case (DFNT_FLOAT32):   /* definition value 5 */
      if ((fdata = (gafloat *)galloc(len * sizeof (gafloat),"fdata"))==NULL) {
    ...
    case (DFNT_FLOAT64):  /* definition value  6 */
      if ((ddata = (gadouble *)galloc(sizeof(gadouble)*len,"ddata")) == NULL) {
    ...
    default:
      sprintf (pout,"HDF coordinate axis data type %d not handled\n",dtype);
      gaprnt(0,pout);
      goto err1;
    };
  ...
  if (pfi->linear[dim]==1) {
    vals = (gadouble *)galloc(sizeof(gadouble)*6,"linearvals");
    ...
    /* JMA make sure this is okay to omit */
/*     if (dim<2 && v2<=0.0) { */
/*       sprintf (pout,"sdfdeflev: axis values for dim=%d have a negative increment\n",dim); */
/*       gaprnt(0,pout); */
/*       goto err1; */
/*     } */
    *(vals)   = v2;
    ...
    *(vals+5) = -999.9;
    ...
    vals = (gadouble *)galloc((pfi->dnum[dim]+5)*sizeof(gadouble),"levelvals");
    if (vals==NULL) goto err1;
...
gaint read_metadata (struct gafile *pfi) {
  struct gavar *pvar;
  gaint rc,i,ii,j,len,natts,ndims,nvars,status,oflg,dummy;
  char name[MAX_NC_NAME+1];
  size_t size=0;
#if USEHDF ==1
  int32 ndsets,dimsize,sds_id,dim_id,dtype,ndatts;
  int32 dim_sizes[MAX_VAR_DIMS],rank;
#endif
  ...
  if (pfi->tmplat) {
    ...
    if (pfi->ncflg==1) {
      rc = gaopnc (pfi,0,1);
      if (rc) {
	gaprnt(0,"read_metadata: gaopnc failed \n");
	return Failure;
      }
    }
#endif
...
#if USEHDF==1
  if (pfi->ncflg==2) {
    status = SDfileinfo(pfi->sdid, &ndsets, &natts);
    if (status == -1) {
	...
	/* get HDF coordinate info */
	dim_id = SDgetdimid(sds_id,0); 
	status = SDdiminfo(dim_id, name, &dimsize, &dtype, &ndatts);
	if (dimsize==0) {
	  /* This is the unlimited dimension */
	  status = SDgetinfo(sds_id, name, &rank, dim_sizes, &dtype, &natts);
	  if (status == -1) {
	    sprintf (pout,"read_metadata: SDgetinfo failed for coordinate %d\n",dim_id);
	    gaprnt(0,pout);
	    goto err3;
	  }
	  /* the first element of the dim_sizes array contains the number of records
	     in the unlimited dimension.*/
          dimsize = dim_sizes[0]; 
	}
	pfi->sdfdimids[ndims] = (gaint)dim_id;
	strcpy(&pfi->sdfdimnam[ndims][0],name);
	...
	status = SDdiminfo(dim_id, name, &dimsize, &dtype, &natts);
	pfi->sdfdimids[ndims] = (gaint)dim_id;
	strcpy(&pfi->sdfdimnam[ndims][0],name);
	pfi->sdfdimsiz[ndims] = (gaint)dimsize;
	ndims++;
...
#if USENETCDF==1
  if (pfi->ncflg==1) {
    status = nc_inq(pfi->ncid, &ndims, &nvars, &natts, NULL);
    if (status != NC_NOERR) {
      handle_error(status);
      goto err1;
    }
...
#if USEHDF==1
  if (pfi->ncflg==2) {
    read_hdfatts (pfi->sdid, "global", natts, pfi);
  }
#endif
#if USENETCDF==1
  if (pfi->ncflg==1) {
    read_ncatts (pfi->ncid, NC_GLOBAL, NULL, natts, pfi); 
  }
#endif
  /* Get variable info and attributes */
  if ((pvar = (struct gavar *)galloc(pfi->vnum * (sizeof(struct gavar)),"pvar")) == NULL) {
  ...
  while (i<pfi->vnum)  {
    ...
    if (pfi->ncflg==2) {
      ...
      sds_id = SDselect(pfi->sdid, i);
      if (sds_id==FAIL) {
	sprintf (pout,"read_metadata: SDselect failed for varid %d\n",i);
	gaprnt(0,pout);
	goto err3;
      }
      status = SDgetinfo(sds_id, name, &(pvar->nvardims), dim_sizes, &dtype, &natts);
      if (status == -1) {
	sprintf (pout,"read_metadata: SDgetinfo failed for varid %d\n",i);
	gaprnt(0,pout);
	goto err3;
      }
      status = SDnametoindex(pfi->sdid, name);
      if (status == -1) {
	sprintf (pout,"read_metadata: SDnametoindex failed for varid %d\n",i);
	gaprnt(0,pout);
	goto err3;
      }
      pvar->sdvid = status;
    ...
    if (pfi->ncflg==1) {
      ...
      if (status != NC_NOERR) {
	sprintf (pout,"read_metadata: nc_inq_var failed to retrieve variable info for varid %d\n",i);
	gaprnt(0,pout);
	handle_error(status);
	goto err3;
      }
...
gaint read_hdfatts (gaint sdid, char *vname, gaint natts, struct gafile *pfi) 
{
  ...
  if (cmpwrd("global",vname)) 
    ...
    len=strlen(vname)+2;
  if ((varname=(char*)galloc(len,"hdfvname"))==NULL) {
  ...
  for (i = 0 ; i < natts ; i++) {
    ...
    if ((attname=(char*)galloc(MAX_NC_NAME+1,"hdatname"))==NULL) {
	...
	switch (attr_dtype) 
	  ...
	  case (DFNT_CHAR8):    /* definition value 4 */
	    len = len + 1; 
	    cval = (char8*) galloc ((len) * sizeof (char8),"catval");
	  ...
	  case (DFNT_UCHAR8):   /* definition value 3 */
	    ucval = (uchar8*) galloc (len * sizeof (uchar8),"ucatval");
	  ...
	  case (DFNT_INT8):     /* definition value 20 */
	    icval = (int8*) galloc (len * sizeof (int8),"iatval");
	  ...
	  case (DFNT_UINT8):    /* definition value 21 */
	    uicval = (uint8*) galloc (len * sizeof (uint8),"uiatval");
	  ...
	  case (DFNT_INT16):    /* definition value 22 */
	    sval = (int16*) galloc (len * sizeof (int16),"satval");
	  ...
	  case (DFNT_UINT16):   /* definition value 23 */
	    usval = (uint16*) galloc (len * sizeof (uint16),"usatval");
	    ...
	    break;
	  case (DFNT_INT32):    /* definition value 24 */
	    ival = (int32*) galloc (len * sizeof (int32),"latval");
	    ...
	    break;
	  case (DFNT_UINT32):   /* definition value 25 */
	    uival = (uint32*) galloc (len * sizeof (uint32),"ulatval");
	    ...
	    break;
	  case (DFNT_FLOAT32):  /* definition value  5 */
	    fval = (float32*) galloc (len * sizeof (float32),"fatval");
	    ...
	    break;
	  case (DFNT_FLOAT64):  /* definition value  6 */
	    dval = (float64*) galloc (len * sizeof (float64),"datval");
	...
	if (gotatt) {
	  ...
	  if ((newattrib = (struct gaattr *) galloc(sizeof(struct gaattr),"newathdf")) == NULL) {
	    sprintf (pout,"read_hdfatts error: memory allocation failed when adding attribute number %d\n",i);
	    gaprnt(2,pout);
...
gaint read_ncatts (gaint cdfid, gaint varid, char *vname, gaint natts, struct gafile *pfi) 
{
  ...
  if ((varid == NC_GLOBAL) && (vname == NULL)) 
    ...
    len = strlen(vname)+2;
  if ((varname=(char*)galloc(len,"ncvname"))==NULL) {
  ...
  for (i=0 ; i<natts ; i++) {
    ...
    if ((attname=(char*)galloc(MAX_NC_NAME+2,"ncattname"))==NULL) {
    ...
    if (status != NC_NOERR) { 
      handle_error(status); 
      sprintf (pout,"read_ncatts: ncattname failed for varid %d attribute number %d\n",varid,i);
      gaprnt(2,pout);
    }
    else {
      len=0; type=0;
      /* Get current attribute's data type and length */
      status = nc_inq_att(cdfid, varid, attname, &type, (size_t*)&len);
      if (status != NC_NOERR) { 
	handle_error(status); 
	sprintf (pout,"read_ncatts: nc_inq_att failed for varid %d attribute number %d\n",varid,i);
	gaprnt(2,pout);
      }
      else {
        if (len > 0) {
	  ...
	  switch (type) 
	    {
	    case (NC_BYTE):
	      bval = (char *) galloc (len*sizeof(char),"bval");
  	      status = nc_get_att_schar(cdfid, varid, attname, (signed char*)bval);
              if (status != NC_NOERR) { 
		gree(bval,"f22"); 
		bval = NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type BYTE\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
	    ...
	    case (NC_CHAR):
	      len = len + 1;
	      cval = (char *) galloc (len*sizeof(char),"cval");
	      status = nc_get_att_text(cdfid, varid, attname, cval);
              if (status != NC_NOERR) { 
		gree(cval,"f24");
		cval = NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type CHAR\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
		cval[len-1]='\0';
	      ...
	      break;
	    case (NC_SHORT):
	      sval = (short *) galloc (len * sizeof(short),"sval");
	      status = nc_get_att_short(cdfid, varid, attname, sval);
              if (status != NC_NOERR) { 
		gree(sval,"f26");
		sval=NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type SHORT\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
	      ...
	      break;
	    case (NC_LONG):
	      ival = (long *) galloc (len * sizeof(long),"ival");
	      status = nc_get_att_long(cdfid, varid, attname, ival);
              if (status != NC_NOERR) { 
		gree(ival,"f28");
		ival = NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type LONG\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
	      ...
	      break;
	    case (NC_FLOAT):
	      fval = (gafloat *) galloc (len * sizeof(gafloat),"fval");
	      status = nc_get_att_float(cdfid, varid, attname, fval);
              if (status != NC_NOERR) {
		gree(fval,"f30");
		fval = NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type FLOAT\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
	      ...
	      break;
	    case (NC_DOUBLE): 
	      dval = (gadouble *) galloc (len * sizeof(gadouble),"dval");
	      status = nc_get_att_double(cdfid, varid, attname, dval);
              if (status != NC_NOERR) { 
		gree(dval,"f32");
		dval = NULL;
		handle_error(status); 
		sprintf (pout,"read_ncatts: failed to get %s attribute %d type DOUBLE\n",varname,i);
		gaprnt(2,pout);
	      }
	      else { 
		gotatt=1; 
            ...
            break;
	    default:
	      sprintf (pout,"read_ncatts: %s attribute %d type %d not supported\n",varname,i,type);
	      gaprnt(2,pout);
	    };
	  
	  if (gotatt) {
	    /* Successfully extracted the attribute, so add a link to the list */
	    if ((newattrib = (struct gaattr *) galloc(sizeof(struct gaattr),"newattr")) == NULL) {
	      sprintf (pout,"read_ncatts: memory allocation failed when adding attribute number %d\n",i);
	      gaprnt(2,pout);
...
#if USEHDF == 1 
  int32 lst,lcnt,sds_id,dtype,rank,dim_sizes[MAX_NC_DIMS],natts;
  float32  fdata;
  int32    idata;
  uint32   uidata;
  ...
  if (pfi->ncflg==2) {
	...
	return Failure;
      ...
      *data = ddata;
      break;
    default:
      sprintf (pout,"SDF coordinate axis data type %d not handled\n",dtype);
      gaprnt(0,pout);
      return Failure;
    };
  }
...
gaint gadxdf(struct gafile *pfi, GASDFPARMS *parms) 
{
  ...
  if (descr == NULL) {
    ...
    sname = (char *)galloc(strlen(pfi->dnam)+5,"sname");
    if (sname == NULL) {
  ...
  while (fgets(rec,512,descr)!=NULL) {
	...
	len = 0;
	while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
	if ((pfi->undefattr = (char *)galloc(len+1,"undefattr2")) == NULL) goto err8;
	...
	*(pfi->undefattr+len) = '\0';
    ...
    /* Parse XDEF */
      ...
      len = 0;
      while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
      if ((parms->xdimname = (char *)galloc(len+1,"xdimname")) == NULL) goto err8;
    ...
    /* Parse YDEF */
      ...
      len = 0;
      while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
      if ((parms->ydimname = (char *)galloc(len+1,"ydimname")) == NULL) goto err8;
    ...
    /* Parse ZDEF */
      ...
      len = 0;
      while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
      if ((parms->zdimname = (char *)galloc(len+1,"zdimname")) == NULL) goto err8;
	...
	len = 0;
	while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
	if ((parms->tdimname = (char *)galloc(len+1,"tdimname")) == NULL) goto err8;
	...
	*(parms->tdimname+len) = '\0';
      ...
      if ((ch = nxtwrd(ch)) == NULL) {
        if (parms->tdimname == NULL) {
	  if ((tvals = (gadouble *)galloc(sizeof(gadouble)*8,"tvals3")) == NULL) goto err8;
	  tvals[0] = 1.0 ;
	  tvals[1] = 1.0 ;
	  tvals[2] = 1.0 ;
	...
	if (parms->tdimname == NULL) {
	  ...
	  if ((tvals = (gadouble *)galloc(sizeof(gadouble)*8,"tvals4")) == NULL) goto err8;
	  *(tvals) = dt1.yr;
	  *(tvals+1) = dt1.mo;
	  *(tvals+2) = dt1.dy;
	  *(tvals+3) = dt1.hr;
    ...
    /* Parse EDEF */
      ...
      len = 0;
      while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
      if ((parms->edimname = (char *)galloc(len+1,"edimname")) == NULL) {
	...
	if (pfi->dnum[EINDEX]<1) {
	  sprintf(pout,"Warning: Invalid EDEF syntax in %s -- Changing size of E axis from %d to 1 \n",
	...
	/* allocate an array of ensemble structures */
	size = pfi->dnum[EINDEX] * sizeof(struct gaens); 
	if ((ens = (struct gaens *)galloc(size,"ens3")) == NULL) {
    ...
    /* Parse the variable declarations */
      ...
      size = pfi->vnum * sizeof(struct sdfnames) ;
      if ((varnames = (struct sdfnames *) galloc(size,"varnames")) == NULL) goto err8;
	...
	if ((strncmp("*:attr",mrec,6)==0) || (strncmp("@",mrec,1)==0)) {
	  ...
	  sprintf (pout,"gadxdf error: Looking for \"endvars\", found \"%s\" instead.\n",rec);
	  gaprnt (0,pout);
	  goto err9;
	}
