...
static unsigned long cvals[120];          /* Color pixel values */
static gaint cused[120];                    /* Color is assigned */
static gaint cmach[120];                    /* Color is matched */
...
char **flist,*xfnam;
...
#if defined (XLIBEMU)
  x11_initialized = 1;    /*ams I need this for keyboard handling ams*/
  if ( xsize >= ysize ) {           /* landscape */
         dw = ( dw >= 800 ) ? 0.9*dw : 0.72*dw;
  } else {                          /* portrait */
         dw = ( dw >= 800 ) ? 0.7*dw*xsz/ysz : 0.56*dw*xsz/ysz;
  }
  dh = (gaint)((gadouble)(dw)*ysz/xsz);

#elif defined(XLIBEMU32)
  if ( xsize >= ysize ) {           /* landscape */
       dw = 0.96 * dw;
       dh = (gaint)((gadouble)(dw)*ysz/xsz);
  } else {                          /* portrait */
  ...
  for (i=0; i<16; i++) {
    ...
    if (XAllocColor(display, cmap, &cell)) {
      cvals[i+100] = cell.pixel;
    } else {
      cvals[i+100] = cvals[15];
    }
    cused[i+100] = 1;
  }
...
gaint gxdacl (gaint clr, gaint red, gaint green, gaint blue) {
  ...
  if (XAllocColor(display, cmap, &cell)) {
    ...
    cused[clr] = 1;
#if !( defined(XLIBEMU) || defined(XLIBEMU32) )
