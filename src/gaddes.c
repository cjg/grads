...
gaint gaddes (char *name, struct gafile *pfi, gaint mflag) {
 ...
 /*mf --- define here vice grads.c for cdunif.c mf*/
  ...
  if (descr == NULL) {
    ...
    if ((sname = (char *)galloc(strlen(name)+5,"sname2")) == NULL) {
  ...
  while (fgets(rec,512,descr)!=NULL) {
	...
	len = 0;
	while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
	if ((pfi->scattr = (char *)galloc(len+3,"scattr")) == NULL) goto err8;
	...
	*(pfi->scattr+len) = '\0';
	...
	if ( (ch=nxtwrd(ch)) == NULL ) {
	  ...
	  len = 0;
	  while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
	  if ((pfi->ofattr = (char *)galloc(len+3,"ofattr")) == NULL) goto err8;
	...
	if ((pfi->bufrinfo = (struct bufrinfo *)galloc(sizeof(struct bufrinfo),"bufrinfo")) == NULL) goto err8;
	...
	if (pfi->bufrinfo->stidxy[0]==-999 || pfi->bufrinfo->stidxy[1]==-999) goto err6a;
      ...
      len = 0;
      while (*(pout+len)) len++;
      if ((pfi->mnam = (char *)galloc(len+3,"mnam")) == NULL) goto err8; 
	...
	len = 0;
	while (*(ch+len)!=' ' && *(ch+len)!='\n' && *(ch+len)!='\t') len++;
	if ((pfi->undefattr = (char *)galloc(len+3,"undefattr3")) == NULL) goto err8;
	...
	*(pfi->undefattr+len) = '\0';
	...
	if ((vectorpairs = (char *)galloc(strlen(ch)+1,"vecpairs")) == NULL) goto err8;
	...
	pfi->dnum[4] = 1;
      ...
      if ((vals = (gadouble *)galloc(sizeof(gadouble)*6,"evals2")) == NULL) goto err8;
      ev1=ev2=1;
      *(vals+1) = ev1 - ev2;
      *(vals) = ev2;
      *(vals+2) = -999.9;
      ...
      *(vals+5) = -999.9;
      ...
      ens = (struct gaens *)galloc(size,"ens4");
      if (ens==NULL) {
	...
	while (!cmpwrd("endedef",rec)) {
	  ...
	  if (!isalnum(*(mrec))) {
	    ...
	    sprintf(pout,"Open Error:  Looking for \"ENDEDEF\", found \"%s\" instead.\n",mrec);
	    gaprnt (0,pout);
	    goto err9;
	  }
	  /* get a new record */
  ...
  if (pfi->ens1==NULL) {
	...
	sprintf(pout,"Open Error:  ensemble %d (%s) extends beyond the time axis limits\n",j,ens->name);
	gaprnt(0,pout);
	goto retrn;
      }
      j++; ens++;
  ...
  if (pfi->idxflg && mflag) {
    ...
    if (mfile==NULL) {
      sprintf (pout,"Open Error:  Can't open Station/Index map file %s \n",pfi->mnam);
      gaprnt (0,pout);
      goto retrn;
    }
	...
	if ((pindx = (struct gaindx *)galloc(sizeof(struct gaindx),"pindx"))==NULL) 
	  goto err8;
	pfi->pindx = pindx;
	...
	if ((vermap == 2) || (vermap == 3)) {        
	  ...
	  pindx->fltpnt = NULL;
	  if (pindx->hinum>0) {
	    if ((pindx->hipnt = (gaint*)galloc(sizeof(gaint)*pindx->hinum,"hipnt"))==NULL) 
	      goto err8;
	    for (i=0; i<pindx->hinum; i++) {
	      rc = fread(urec,sizeof(unsigned char),4,mfile);
	      if (rc!=4) { 
		sprintf(pout,"Error reading integer %d from header of GRIB1 index file\n",i); 
		gaprnt(0,pout); 
		goto retrn; 
	      }
	      idum = gagby(urec,0,4);
	  ...
	  if (pindx->hfnum>0) {
	    if ((pindx->hfpnt = (gafloat*)galloc(sizeof(gafloat)*pindx->hfnum,"hfpnt"))==NULL) 
	      goto err8; 
	  ...
	  if (pindx->intnum>0) {
	    if ((pindx->intpnt = (gaint*)galloc(sizeof(gaint)*pindx->intnum,"intpnt"))==NULL) 
	      goto err8; 
	    for (i=0; i<pindx->intnum; i++) {
	      rc = fread(urec,sizeof(unsigned char),4,mfile);
	      if (rc!=4) { 
		sprintf(pout,"Error reading integer %d from GRIB1 index file\n",i); 
		gaprnt(0,pout); 
		goto retrn; 
	      }
	      idum = gagby(urec,0,4);
	  ...
	  if (pindx->fltnum>0) {
	    if ((pindx->fltpnt = (gafloat *)galloc(sizeof(gafloat)*pindx->fltnum,"fltpnt"))==NULL) 
	      goto err8; 
	    for (i=0; i<pindx->fltnum; i++) {
	      rc = fread(urec,sizeof(unsigned char),4,mfile);
	      if (rc!=4) { 
		sprintf(pout,"Error reading float %d from GRIB1 index file\n",i); 
		gaprnt(0,pout); 
		goto retrn; 
	      }
	      fdum = ibm2flt(urec);
	...
	if (g2indx->version == 1) {     
	  ...
	  if (g2indx->g2intnum>0) {
	    if ((g2indx->g2intpnt = (gaint *)galloc(sizeof(gaint)*g2indx->g2intnum,"g2intpnt")) == NULL) 
	      goto err8;
	    rc = fread(g2indx->g2intpnt,sizeof(gaint),g2indx->g2intnum,mfile);
	    if (rc!=g2indx->g2intnum) {
	      sprintf(pout,"Error reading GRIB2 index file, rc=%d\n",rc);
	      gaprnt(0,pout);
	      goto retrn;
	    }
	  ...
	  sprintf(pout,"Unknown GRIB2 index version number: %d\n",g2indx->version);
	  gaprnt(0,pout);
	  goto retrn;
	}
      }
	...
	if (gagbb(urec,0,1)) idum=-idum;
	maxlv=idum;
	if ((pfi->tstrt = (gaint *)galloc(sizeof(gaint)*mcnt,"tstrt")) == NULL) goto err8;
	if ((pfi->tcnt  = (gaint *)galloc(sizeof(gaint)*mcnt,"tcnt")) == NULL) goto err8;
	for(i=0;i<mcnt;i++) {
	  fread(urec,sizeof(unsigned char),4,mfile);
	  idum=gagby(urec,0,4);
	  if (gagbb(urec,0,1)) idum=-idum;
	...
	if ((pfi->tstrt = (gaint *)galloc(sizeof(gaint)*mcnt,"tstrt1")) == NULL) goto err8;
	if ((pfi->tcnt  = (gaint *)galloc(sizeof(gaint)*mcnt,"tcnt1")) == NULL) goto err8;
  ...
  if (vectorpairs) {
    vplist = vectorpairs; 
    if ((pair = (char *)galloc(strlen(vplist)+1,"pair")) == NULL) {
...
/* set the global calendar and check if we are trying to change with a new file...
  ...
  if (mflag) {
    if (pfi->type > 1) {
      ...
      pfi->sbuf = (gafloat *)galloc(size,"sbuf1");
      if (pfi->sbuf==NULL) {
	...
	goto err8;
      ...
      pfi->rbuf = (gadouble *)galloc(size,"rbuf1");
      if (pfi->rbuf==NULL) {
	...
	goto err8;
      ...
      if (pfi->idxflg) {
	pfi->pbuf = (unsigned char *)galloc(size,"pbuf1");
	if (pfi->pbuf==NULL)  {
    ...
    pfi->ubuf = (char *)galloc(size,"ubuf1");
    if (pfi->ubuf==NULL) {
...
struct gaattr *parseattr (char *ch) {
  ...
  if (nctype <= 2) {
    if ((value = (void *)galloc((len+1) * sizeof(char),"valuec")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    strcpy(value,attrvalue);
  } else if (nctype == 3) {
    if ((svalues = (short *)galloc(len * sizeof(short),"values")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    value = (void*)svalues;
    ...
    if ((lvalues = (long *)galloc(len * sizeof(long),"valuel")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    value = (void*)lvalues;
    ...
    if ((fvalues = (gafloat *)galloc(len * sizeof(gafloat),"valuef")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    value = (void*)fvalues;
    ...
    if ((dvalues = (void *)galloc(len * sizeof(gadouble),"valued")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    value = (void*)dvalues;
    ...
    sprintf(pout,"Error: attribute data type not handled: nctype = %d \n",nctype);
    gaprnt(0,pout);
    goto err;
  }
  
  /* Everything parsed and allocated OK, so allocate and populate a gaattr structure */
  attrib = (struct gaattr *) galloc(sizeof(struct gaattr),"attrib");
  if (attrib != NULL) {
...
gaint i;
  ...
  pfi = (struct gafile *)galloc(sizeof(struct gafile),"pfi");
...
gaint *ioff, rdw, rc, pnum, wflg;
  ...
  if (pfi->ppflag != 8) {
    if ((pfi->ppi[0] = (gaint*)galloc(sizeof(gaint)*size,"ppi0")) == NULL) goto merr;
    if ((pfi->ppf[0] = (gadouble*)galloc(sizeof(gadouble)*size,"ppf0")) == NULL) goto merr;
    if ((pfi->ppf[1] = (gadouble*)galloc(sizeof(gadouble)*size,"ppf1")) == NULL) goto merr;
    if (pfi->ppwrot) {
      if ((pfi->ppw  = (gadouble *)galloc(sizeof(gadouble)*size,"ppw")) == NULL) goto merr;
    }
  }
  ...
  if (pfi->ppflag==7) {
    ...
    if ((fvals = (gafloat*)galloc(sizeof(gafloat)*size,"ppfvals")) == NULL) goto merr;
  ...
  /* When pdef is a file, read in the offsets of the points to use and their weights, 
    ...
    for (i=0; i<pnum; i++) {
      ...
      if ((pfi->ppi[i] = (gaint*)galloc(sizeof(gaint)*size,"ppi3")) == NULL) goto merr;
      /* sequential -- header */
	...
	gabswp((gafloat *)(pfi->ppi[i]),size);
      ...
      if ((pfi->ppf[i] = (gadouble*)galloc(sizeof(gadouble)*size,"ppf2")) == NULL) goto merr;
      /* sequential -- header */
      if (pdefop1==2) { rc = fread(&rdw, sizeof(gaint), 1, pdfi); if (rc!=1) goto merr2; }
    ...
    if ((pfi->ppw = (gadouble *)galloc(sizeof(gadouble)*size,"ppw2")) == NULL) goto merr;
    /* sequential -- header */
    if (pdefop1==2) { rc = fread(&rdw, sizeof(gaint), 1, pdfi); if (rc!=1) goto merr2; }
    rc = fread(fvals, sizeof(gafloat), size, pdfi); if (rc!=size) goto merr2;
    /* byte swap if necessary */
