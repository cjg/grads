...
gaint dappfi (char *url, struct gafile *pfi) {
  ...
  gaint i,isvert,nvars,nivars,size,lcnt,len,gotfill,tvar,tminid,tsizeid,tstepid,trngid;
  ...
  pvar = (struct gavar *)galloc(size,"dapstnpvar");
  if (pvar==NULL) {
...
/*   pfi->undef =  1e+35;  */
  ...
  vals = (gadouble *)galloc(sizeof(gadouble)*8,"dapstnvals");
...
gaint dapget(struct gastn *stn) {
  ...
  for (i = 0; i < nreps; i++) {
    ...
    if (pvar->levels==0) {
      ...
      rpt->tim = time;
      rpt->lev = -9.99e33;
      /* Do the fuzzy test for undef values  */
      if (val >= ulow && val <= uhi ) {
	rpt->umask = 0;
	rpt->val = pfi->undef;   
      }
